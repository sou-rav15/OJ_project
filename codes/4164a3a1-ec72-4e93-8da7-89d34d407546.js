...
int main() {
    int n;
    std::cout << "Enter the size of the array (n+1 elements): ";
    std::cin >> n;
    
    if (n <= 1) {
        std::cerr << "Invalid size. Array size must be at least 2." << std::endl;
        return 1;
    }
    
    std::vector<int> nums(n);
    
    std::cout << "Enter " << n << " integers (in the range 1 to " << (n - 1) << "):" << std::endl;
    for (int i = 0; i < n; ++i) {
        std::cin >> nums[i];
    }
